...
### 0.3. Оточення `github-pages` — нічого робити не треба

Раніше тут стояла вимога дозволити тегам деплоїти в це оточення. **Вона більше не потрібна**:
сайт не деплоїться з реліз-рану взагалі (див. 4.9), тож деплой завжди йде з `main`, а це
...
— значить десь лишилась спроба деплоїти сайт із тега. Правильна реакція — прибрати цю спробу,
а не додавати правило.
...
Тригери: пуш у `main` (з `paths`-фільтром) і `workflow_dispatch` для ручного оновлення:
...
**Деплою сайту з реліз-рану тут немає, і це навмисно.** Спроба зробити його через `workflow_call`
не дала результату: джоба генерувала правильний маніфест і звітувала `success`, а сайт віддавав
попередній вміст — двічі поспіль. Замість цього сторінка завантажень запитує реліз у GitHub
**з браузера відвідувача**, тож її коректність не залежить від деплою взагалі. Деталі — 4.9.
...
| 4.9 | сайт не оновлюється після релізу | реліз створив `GITHUB_TOKEN` — від його подій воркфлоу не запускаються; виклик Pages з реліз-рану звітує `success`, але вміст не стає живим | не деплоїти сайт із релізу: сторінка читає реліз у рантаймі, зібраний маніфест — лише фолбек |
...
- [ ] сторінка завантажень читає реліз у рантаймі; деплою сайту з реліз-рану немає
...
- [ ] Pages увімкнені — правило для тегів в оточенні не потрібне (див. 0.3)
