...
| 4.22 | після ввімкнення апдейтера в списку завантажень з'явились `.sig` і `latest.json` | реліз з підписом вантажить їх як звичайні ассети, а платформного правила для них немає — усі падають у `windows` | фільтрувати: `!name.endsWith('.sig') && name !== 'latest.json'` |
| 4.23 | збірка падає з `A public key has been found, but no private key` | секрети підпису задані лише на кроці `tauri-action`, а інший крок теж робить збірку | виносити `TAURI_SIGNING_*` на рівень джоби, а не кроку |
