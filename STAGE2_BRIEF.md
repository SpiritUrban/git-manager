...
| Сайт оновлюється сам після релізу | версія на кнопках = версія релізу, без передеплою |
...
14. **Не намагатися оновити сайт з реліз-рану — сторінка має брати реліз сама.**
    `release: types: [published]` як тригер не працює: реліз створює `GITHUB_TOKEN`, а GitHub
    навмисно не запускає воркфлоу від подій цього токена. Обхід через `workflow_call` із
    реліз-рану **теж не працює**: перевірено на двох релізах — джоба генерувала правильний
    маніфест і звітувала `success`, а сайт продовжував віддавати попередній вміст.
    Робоче рішення: сторінка завантажень запитує реліз у GitHub **з браузера відвідувача**, а
    зібраний маніфест лишається фолбеком. Тоді коректність не залежить від деплою взагалі.
...
| 3 | ~~Правило для тегів в оточенні `github-pages`~~ — **не потрібне**, сайт деплоїться лише з `main` (розділ 10) | — | якщо бачите `Tag ... is not allowed to deploy`, десь лишився деплой сайту з тега — прибрати його, а не додавати правило |
...
# Правило 14: деплою сайту тут навмисно немає.
```
...
## 10. Чому сайт не деплоїться з реліз-рану

Спокуса зробити так велика, тому тут причина, а не лише заборона.

`release: types: [published]` як тригер не спрацює ніколи: реліз створює `GITHUB_TOKEN`, а GitHub
навмисно не запускає воркфлоу від подій цього токена.

Обхід через `workflow_call` із реліз-рану виглядає правильним і **теж не працює**. Перевірено на
двох релізах поспіль: джоба генерувала коректний маніфест (це видно з `::notice`), викладала
артефакт і звітувала `success`, запис деплою з тега був активним, а деплой з `main` — неактивним.
І попри все це сайт продовжував віддавати файли попереднього деплою. Причину встановити не
вдалося: `/repos/.../pages` і логи джоби без авторизації закриті.

**Рішення, яке працює:** сторінка завантажень запитує реліз у GitHub безпосередньо з браузера
відвідувача, а зібраний маніфест лишається фолбеком для тих, хто до API не достукається.
Перевірено в бойових умовах: задеплоєний маніфест показував попередню версію, а кнопки — поточну.

Наслідки для конструкції:
- у `release.yml` **немає** джоби деплою сайту;
- у `pages.yml` **немає** `workflow_call`, лишаються пуш у `main` за `paths` і ручний запуск;
- маніфест на сайті може відставати на версію — це нормально, він лише резерв;
- правило для тегів в оточенні `github-pages` не потрібне — деплой завжди з `main`.
...
- [ ] сторінка завантажень читає реліз у рантаймі; у `release.yml` немає джоби деплою сайту
